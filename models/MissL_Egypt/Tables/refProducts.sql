select pro.*,ColLev.product_type CollectionLevel, cat.category as category_L2 ,
...
--prid.sku variantSku,
--max(case when attribute_code like 'created_at' then value end) launch_date,
-- max(case when attribute_code like 'brand_product' then value_id end) category_L1_id,
--max(case when attribute_code like 'brand_product' then value end) category_L1,
-- max(case when attribute_code like 'key_category' then value_id end) category_L2_id,
--max(case when attribute_code like 'key_category' then value end) category_L2,
max(case when attribute_code like 'candere_product_type' then value end) category_L2,
-- max(case when attribute_code like 'key_subcategory' then value_id end) category_L3_id,
-- max(case when attribute_code like 'key_subcategory' then value end) category_L3,
...
--max(case when  attr.attribute_id=374 then value end) color
--max(case when  attr.attribute_id=443 then value end) CollectionLevel
from
(
select
attr.value_id,
...
(
select value_id, attribute_id, entity_id, cast(value as string) value from `noted-computing-279322.halo_1_1_Egypt.magento_productentityvarchar`
...
`noted-computing-279322.halo_1_1_Egypt.magento_productentityint` atr

left join

`noted-computing-279322.halo_1_1_Egypt.magento_producteavoption` op
on atr.attribute_id = op.attribute_id
and atr.value = op.option_id
left join
(select * from `noted-computing-279322.halo_1_1_Egypt.magento_producteavoptionvalue` opval

where value_id = (select max(value_id)

from `noted-computing-279322.halo_1_1_Egypt.magento_producteavoptionvalue` opval1 where
...
--`daton-candere.halo_1_0.magento_productentity` prid,
(select attribute_id, attribute_code
from `noted-computing-279322.halo_1_1_Egypt.magento_producteav`
...
--where prid.entity_id=pr.entity_id and
where pr.attribute_id= attr.attribute_id
group by pr.entity_id)base1

left join

(select distinct parent_id,child_id from `noted-computing-279322.halo_1_1_Egypt.magento_productrelation` ) par
on base1.variantid=par.child_id
) ))base2

left join

(select entity_id,sku as parent_sku from `noted-computing-279322.halo_1_1_Egypt.magento_productentity`) parsku
...
(select entity_id,value from `noted-computing-279322.halo_1_1_Egypt.magento_productentitytext`
where attribute_id=374)color_id

left join

(select max(value) as value,option_id from `noted-computing-279322.halo_1_1_Egypt.magento_producteavoptionvalue`
...
(select min(date(created_at)) as launch,product_id as product_id1 from `noted-computing-279322.halo_1_1_Egypt.magento_transaction_details`
...
from `noted-computing-279322.halo_1_1_Egypt.magento_transaction_details`
) b
...
left join
 ...
 `noted-computing-279322.halo_1_1_Egypt.magento_missl_productscat`  cat
...
FROM
`noted-computing-279322.halo_1_1_Egypt.magento_productentityint` catalog_product_entity_int
     INNER JOIN
`noted-computing-279322.halo_1_1_Egypt.magento_producteavoptionvalue`  eav_attribute_option_value
