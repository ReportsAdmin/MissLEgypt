...
from `noted-computing-279322.halo_1_1_Egypt.Calendar` a
cross join
`noted-computing-279322.halo_1_1_Egypt.magento_influencerscode` b) c
left join `noted-computing-279322.halo_1_1_Egypt.magento_influencerscode` d
on c.code = lower(trim(d.code))
and lower(c.monthname) = lower(substr(d.month_paid,1,3))
and c.year = d.year
where lower(c.country_ksa_egypt) = 'egypt') e
left join `noted-computing-279322.halo_1_1_Egypt.fOrders` f
on e.date = f.order_date
and e.code = lower(f.coupon_code))
group by 1,2)) x
left join `noted-computing-279322.halo_1_1_Egypt.fDailyAgg` y
on x.date = y.date))
