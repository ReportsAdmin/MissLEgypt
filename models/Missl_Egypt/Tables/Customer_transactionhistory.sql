select *, 'MissLEgypt' Halo_Country
from (
...
From `noted-computing-279322.halo_1_1_Egypt.magento_transactionnew` ord
left join `noted-computing-279322.halo_1_1_Egypt.magento_address` address
...
(
Select user_id, count(distinct order_id) as orders, sum(total_item_price) as revenue from `noted-computing-279322.halo_1_1_Egypt.fOrders`
where order_status = 'successful'
group by 1))
cross join
(select spend/unique_cust as CAV
from
((Select sum(spend) spend  from `noted-computing-279322.halo_1_1_Egypt.fAdInsights`)
cross join
(select count (distinct user_id) unique_cust from `noted-computing-279322.halo_1_1_Egypt.fOrders`
where order_status = 'successful')))
)
