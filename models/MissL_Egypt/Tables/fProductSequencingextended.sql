select *
from (select *,row_number() over (partition by category_L2 order by AverageRev desc) DesiredPositioning,'MissLEgypt' Halo_Country
        from(select a.*,safe_divide(Revenue,30) AverageRev,activationdate.activation_date
                          from (select a.*,c.CatlogSequence
                                from (select distinct product_id,
                                        ...
                                        sum(units) units,
                                        sum(revenue) revenue
                                        from `noted-computing-279322.halo_1_1_Egypt.fProductInsightsextended`
                          ...
                          left join (select distinct entity_id,min(date(created_at)) as Activation_date
                                      from `noted-computing-279322.halo_1_1_Egypt.magento_productentity`
