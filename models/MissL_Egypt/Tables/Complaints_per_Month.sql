...
(Select a.*, b.* from (
(Select distinct FORMAT_DATE("%b-%Y", Date) as Ticket_Open from `noted-computing-279322.halo_1_1.zendesk_users`) a
join (Select distinct FORMAT_DATE("%b-%Y", Date) as Ticket_closed from `noted-computing-279322.halo_1_1.zendesk_users`) b
...
ifnull(count(distinct Requester_ID),0) as Complaints FROM `noted-computing-279322.halo_1_1.zendesk_users`
