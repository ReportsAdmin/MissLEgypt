select distinct ifnull(trim(lower(city)),'unknown') as city,date, ifnull(sum(cancelled_prod_quantity),0) as cancelled_orders,ifnull(sum(cancelled_prod_quantity)*100/sum(allorders_prod_quantity),0) as percentage_cancellation,ifnull(sum(allorders_prod_quantity),0) as sum_all_orders_product_quantity from
(select
pic,(sku_name),date_level_1 as date, city,cancelled_prod_quantity, allorders_prod_quantity*100/(allorders_quantity) as percentage_overall_quantity, successful_prod_quantity*100/ total_successful_quantity as percentage_successful_quantity, cancelled_prod_quantity*100/total_cancelled_quantity as percentage_cancelled_quantity,successful_prod_quantity,allorders_prod_quantity
from
(select distinct(cancelled.sku) as sku_name, sum(cancelled_prod_quantity) over(partition by cancelled.date_level_1) as total_cancelled_quantity,sum(successful_prod_quantity) over(partition by successful.date_level_1) as total_successful_quantity,sum(allorders_prod_quantity) over(partition by allorders.date_level_1) as allorders_quantity, cancelled.city as city,pic,cancelled.date_level_1,cancelled_prod_quantity,successful_prod_quantity,allorders_prod_quantity
from
(
-- cancelled orders
...
from
-- main data from A
((select product_sku, increment_id, product_id,order_status,order_date, product_quantity,city from `noted-computing-279322.halo_1_1_Egypt.fOrders` as  orders) ord
left join
