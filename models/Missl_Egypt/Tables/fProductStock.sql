...
(select distinct entity_id from `noted-computing-279322.halo_1_1_Egypt.magento_productentityint` ) entity
left join
(select distinct product_id,quantity as qty from `noted-computing-279322.halo_1_1_Egypt.magento_stock_status` where quantity>0)inv
on entity.entity_id=inv.product_id) base1
left join
(select distinct  entity_id,value as base_mrp from `noted-computing-279322.halo_1_1_Egypt.magento_productentitydec` where attribute_id= 74   ) mrp  -- 75
on base1.parent_id=mrp.entity_id
left join
(select distinct  entity_id, max(value ) base_price from `noted-computing-279322.halo_1_1_Egypt.magento_productentitydec`
